...
const TOOLS = [
  ...
  { name: 'docs_knowledge', description: 'БАЗА ЗНАНИЙ направления документов. Лаборатории (у Серконс ~20 лабораторий, у сотрудницы Кати — свои, цены другие) с СЕБЕСТОИМОСТЯМИ по услугам; растаможка под ключ по таможенным постам. action: add_lab (добавить/ИЗМЕНИТЬ лабораторию и её себестоимости — «запомни лабораторию X, себес СБКТС+ЭПТС 20000» или «поменяй себес в Голицыно на 17000»; costs дополняются, не стираются), remove_lab (убрать лабораторию — «убери/отключи лабораторию Люберцы»), add_customs (растаможка по посту), list (вся база). Себестоимость из базы сама подставляется в заказы CRM.', input_schema: { type: 'object', properties: { action: { type: 'string', enum: ['add_lab', 'remove_lab', 'add_customs', 'list'] }, name: { type: 'string', description: 'название лаборатории (точно как в базе для изменения/удаления)' }, owner: { type: 'string', description: 'Серконс / Катя / другое' }, costs: { type: 'object', description: 'себестоимости {"СБКТС+ЭПТС":20000,"ЭПТС":5000}' }, post: { type: 'string', description: 'таможенный пост' }, price: { type: 'number', description: 'цена под ключ' }, includes: { type: 'string', description: 'что входит' }, notes: { type: 'string' } }, required: ['action'] } },
  { name: 'content_ideas', description: 'Показать идеи контента, которые мозг-маршрутизатор отобрал из партнёрских каналов по направлениям docs (растаможка/СБКТС/утиль — темы для инфо-роликов @LegalAuto24) и parts (запчасти @LegalAutoParts24). Авто идут отдельно на одобрение сразу. Вызывай на «какие идеи / что накопилось / темы для роликов из каналов». action clear — очистить.', input_schema: { type: 'object', properties: { action: { type: 'string', enum: ['list', 'clear'] } } } },
  { name: 'docs_quote', description: 'ПРОСЧЁТ услуги документов для клиента: находит где себестоимость дешевле (по всей базе Кати+Серконс) и предлагает цену клиенту с наценкой + показывает твою маржу. Вызывай на «посчитай клиенту СБКТС+ЭПТС», «сколько взять за ЭПТС в Москве», «где дешевле утиль». service — тип услуги как в базе (СБКТС+ЭПТС, ЭПТС, Утиль коммерческий, ГЛОНАСС...). region — город/лаборатория (необязательно). markup — твоя наценка в ₽ (по умолчанию 10000).', input_schema: { type: 'object', properties: { service: { type: 'string' }, region: { type: 'string' }, markup: { type: 'number' } }, required: ['service'] } },
  ...
  try {
    switch (name) {
      ...
      case 'docs_knowledge': {
        ...
        if (input.action === 'remove_lab') {
          if (!input.name) return 'Укажи название лаборатории для удаления.';
          const n = kb.removeLab(input.name);
          return n ? `Убрал лабораторию «${input.name}» из активных.` : `Лаборатория «${input.name}» не найдена. Посмотри точное название через «покажи базу знаний».`;
        }
      ...
      case 'content_ideas': {
        const { getSection, setSection } = await import('../services/stateService.js');
        if (input.action === 'clear') { setSection('content_ideas', { list: [] }); return 'Очередь идей очищена.'; }
        const list = (getSection('content_ideas') || {}).list || [];
        if (!list.length) return 'Идей пока нет. Маршрутизатор наполнит, когда подключишь каналы и пойдут посты по растаможке/документам/запчастям.';
        const byCat = { docs: [], parts: [] };
        for (const it of list) (byCat[it.category] || (byCat[it.category] = [])).push(it);
        const fmt = arr => arr.slice(0, 8).map(i => `• ${i.why} — ${i.text.slice(0, 70)}… (${i.source})`).join('\n');
        return [
          byCat.docs?.length ? `📄 ДОКУМЕНТЫ/РАСТАМОЖКА → @LegalAuto24 (темы для инфо-роликов):\n${fmt(byCat.docs)}` : '',
          byCat.parts?.length ? `🔧 ЗАПЧАСТИ → @LegalAutoParts24:\n${fmt(byCat.parts)}` : '',
        ].filter(Boolean).join('\n\n') || 'Идей нет.';
      }
      case 'docs_quote': {
        const kb = await import('../services/docsCrm.js');
        const q = kb.quoteService(input.service, { region: input.region || '', markup: input.markup ?? 10000 });
        if (!q.options.length) return `По услуге «${input.service}»${input.region ? ` в «${input.region}»` : ''} нет себестоимости в базе. Проверь название услуги («покажи базу знаний») или добавь цену.`;
