...
"${String(text).slice(0, 1100)}"` }] });
  ...
  // Наличие: ЧЕСТНО. Любой намёк на привоз/пригон → «под заказ» (недо-обещать безопаснее,
  // чем соврать «уже в РФ»). «В наличии» ставим ТОЛЬКО если явно в РФ и НЕТ признаков привоза.
  const low = String(text).toLowerCase();
  const importSig = /под заказ|под привоз|в пути|пригон|привоз|привез|срок поставк|доставк|растаможк|из кита|из коре|из япон|из оаэ|из европ|из дуба|заказ из|под ключ из|китай|корея/.test(low);
  const stockSig  = /в наличии в москв|в наличии в рф|в наличии в росси|уже в москв|уже в рф|уже в росси|можно приехать|можно посмотреть сегодня|на нашей площадк|забирай сегодня/.test(low);
  d.avail = (stockSig && !importSig) ? 'stock' : 'order';   // привоз побеждает всегда → по умолчанию order
  ...
  // turbo* — фонк с студийной прокачкой (бас-буст, компрессия, +6% темп, клиповая громкость)
  const music = pickMusic(['turbo']);
