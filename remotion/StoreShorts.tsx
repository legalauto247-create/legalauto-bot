...
const Photo: React.FC<{ src: string; dur: number; mode?: 'in' | 'pan' | 'out' }> = ({ src, dur, mode = 'in' }) => {
  ...
  const scale = mode === 'out' ? interpolate(f, [0, dur], [1.22, 1.08]) : interpolate(f, [0, dur], [1.08, 1.24]);
  const panX = mode === 'pan' ? interpolate(f, [0, dur], [-30, 30]) : 0;
  const url = /^https?:/.test(src) ? src : staticFile(src);
  return <Img src={url} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${scale}) translateX(${panX}px)` }} />;
};
  ...
  return (
    <AbsoluteFill style={{ background: '#0A0A0A' }}>
      ...
      <Sequence durationInFrames={F[1]}>
        <Photo src={ph(0)} dur={F[1]} mode="in" /><Scrim />
      ...
      <Sequence from={F[1]} durationInFrames={F[2] - F[1]}>
        <Photo src={ph(1)} dur={F[2] - F[1]} mode="pan" /><Scrim />
      ...
      <Sequence from={F[2]} durationInFrames={F[3] - F[2]}>
        <Photo src={ph(2)} dur={F[3] - F[2]} mode="in" /><Scrim />
      ...
      <Sequence from={F[3]} durationInFrames={F[4] - F[3]}>
        <Photo src={ph(3)} dur={F[4] - F[3]} mode="out" /><Scrim />
        <AbsoluteFill style={{ justifyContent: 'flex-start', padding: '220px 70px 0', textAlign: 'center', alignItems: 'center' }}>
      ...
      <Sequence from={F[4]} durationInFrames={F[5] - F[4]}>
        <Photo src={ph(4)} dur={F[5] - F[4]} mode="pan" /><Scrim />
        <AbsoluteFill style={{ padding: '190px 80px 0' }}>
      ...
      <Sequence from={F[5]} durationInFrames={F[6] - F[5]}>
        <Photo src={ph(0)} dur={F[6] - F[5]} mode="out" /><Scrim />
        <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '0 70px' }}>
          ...
          <In delay={10}>
            <div style={{ marginTop: 40, display: 'inline-flex', alignItems: 'center', gap: 12, background: ACC, color: '#0A0A0A', fontFamily: FONT, fontWeight: 700, fontSize: 40, padding: '20px 42px', borderRadius: 16, boxShadow: `0 10px 44px ${ACC}55` }}>✈ {channel}</div>
          </In>
